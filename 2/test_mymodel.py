...
for m in range(Power_Test_noisy.shape[0]):
    row = []

    for i in range(nVars):
        row.append(Power_Test_noisy[m, i]**2)
    ...
    for col in range(Cl.shape[1]):
        ...
        row.append(Power_Test_noisy[m, bus_i] * Power_Test_noisy[m, bus_j])
...
time_test = Power_Test_noisy.shape[0]
...
for m in range(time_test):
    ...
    PL_test_eq15[m] = np.dot(Power_Test_noisy[m,:],
                             np.dot(alfa, Power_Test_noisy[m,:].T))

    teta_test[:,m] = np.dot(np.linalg.inv(Bsys), Power_Test_noisy[m,:].T)

    grau_test[:,m] = np.dot(Cl.T, teta_test[:,m])

    PL_test_eq13[m] = np.dot(2*Gv, 1 - np.cos(grau_test[:,m]))

    PT_test[m] = np.sum(Power_Test_noisy[m,:])
