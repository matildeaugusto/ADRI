...
for m in range(time):
    PL[m] = np.dot(Pl[m,:], np.dot(G, Pl[m,:]))
...
Ptest_noisy = Ptest *(1 + noiseFactor * np.random.normal(size=Ptest.shape))

# Discover beta for both the original and noisy power losses

# B = np.linalg.lstsq(X, PL, rcond=None)[0]
B = np.linalg.lstsq(X, PL_noisy, rcond=None)[0]
Btest = np.linalg.lstsq(X, PL_noisy, rcond=None)[0]

# B, residuals, rank, s = np.linalg.lstsq(X, PL, rcond=None)
Breg = B
...
plt.plot(PL_test_pred, 's--', label='Perdas previstas (PL_pred)', linewidth=2)
...
plt.step(range(time), PL, label='True Losses', where='post')
plt.step(range(time), PL_test_pred, label='Predicted Losses', where='post')
plt.xlabel('Time')
...
error_percent = abs((PL_test_pred - PL) / PL * 100)
plt.figure(figsize=(12, 6))
